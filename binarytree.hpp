...
bool BinaryTree<T>::isOrderedIterative() const
{
    ...
    for (typename TreeTraversal<T>::Iterator iterator = inOrder.begin(); iterator != inOrder.end(); ++iterator) {
        if(!first)
            if((*iterator)->elem <= temp)
                ordered = false;
...
template <typename T>
void BinaryTree<T>::isOrderedRecursive(const Node* subRoot, std::vector<T> &v) const {
    if(subRoot == NULL)
        return;
