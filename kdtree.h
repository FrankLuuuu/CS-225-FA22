...
class KDTree
{
  ...
  public:
    ...
    typename KDTree<Dim>::KDTreeNode* helpConstruct(vector<Point<Dim>>& newPoints, int left, int right, int d);

    Point<Dim> quickselect(vector<Point<Dim>>& newPoints, int left, int right, int k, int dim);

    int partition(vector<Point<Dim>>& newPoints,int left, int right, int pivotIndex, int dim);
    
    void copy(KDTreeNode* node, KDTreeNode* otherNode);
    ...
    void findHelp(KDTreeNode* curRoot, Point<Dim> &currentBest, Point<Dim>& query, int dim) const;
